...
static int r8c_thld_set(struct r8c_hwmon *h, int index, long temp)
{
	...
	ret = r8c_exec_cmd(h->r8c, CMD_THSET, cmdbuf, retbuf, 4);
	/* R8C returns '1' when failed to set value */
	if (ret || retbuf[0] == '1') {
		pr_err("r8c-hwmon: failed to set threshold\n");
		return ret;
	}
...
static int r8c_thld_get(struct r8c_hwmon *h, u32 *val)
{
	...
	ret = r8c_exec_cmd(h->r8c, CMD_THGET, NULL, retbuf, 12);
	if (ret) {
		pr_err("r8c-hwmon: failed to get threshold\n");
		return ret;
	}
...
static int r8c_temp_get(struct r8c_hwmon *h, long *temp)
{
	...
	ret = r8c_exec_cmd(h->r8c, CMD_TEMP, NULL, retbuf, 4);
	if (ret) {
		pr_err("r8c-hwmon: failed to get temperature\n");
		return ret;
	}

	return kstrtol(retbuf, 16, temp);
}
				     ...
				     const char *buf, size_t len)
{
	...
	if (index == 0 && thld <= FIELD_GET(0xff00, cur_thld)) {
		dev_err(dev,
			"please specify higher value than speed_temp1\n");
		return -EINVAL;
	}

	/* if setting 1st threshold */
	if (index == 1 && FIELD_GET(0xff0000, cur_thld) <= thld) {
		if (h->model->fan_spd_num <= 3) {
			...
			dev_err(dev,
				"please specify lower value than speed_temp2\n");
			return -EINVAL;
		}
	}
			     ...
			     char *buf)
{
	...
	ret = r8c_exec_cmd(h->r8c, CMD_FANCTL, NULL, retbuf, 4);
	if (ret)
		return 0;
			      ...
			      const char *buf, size_t len)
{
	...
	ret = r8c_exec_cmd(h->r8c, CMD_FANCTL, cmdbuf, NULL, 0);
			   ...
			   unsigned long evcode, void *data)
{
	 ...
	 */
	case R8C_EV_FANERR:
		pr_err("r8c-hwmon: FAN error detected, please check!");
	...
	case R8C_EV_TMPERR:
		...
		pr_err("r8c-hwmon: Temperature error detected!\n");

		if (r8c_thld_get(h, &thld) || r8c_temp_get(h, &temp))
			return NOTIFY_STOP;

		pr_err("r8c-hwmon: (current: %ld, threshold: %u)\n",
...
static int landisk_r8c_hwmon_probe(struct platform_device *pdev)
{
	...
	struct r8c_hwmon *h;
	u32 model_id = *(u32 *)dev_get_platdata(dev->parent);
	int i, ret;

	pr_info("R8C hwmon driver for I-O DATA LAN DISK series\n");

	dev_info(dev, "got model id %d\n", model_id);
	...
	ret = devm_r8c_register_event_notifier(dev, &h->nb);
	if (ret)
		return ret;
