			     ...
			     enum led_brightness brns, bool blink)
{
	...
	return r8c_exec_cmd(data->l->r8c, CMD_HDD, arg, NULL, 0);
}
			     ...
			     u8 max_leds)
{
	...
	if (of_get_child_count(dev->of_node) > max_leds) {
		dev_err(dev, "too many LEDs defined (max: %u)\n", max_leds);
		return -ENODEV;
	}
	...
	device_for_each_child_node(dev, child) {
		...
		if (port > max_leds - 1) {
			dev_warn(dev,
				 "port %d exceeds maximum port number %u, skipping...\n",
				 port, max_leds - 1);
			continue;
		}
		...
		if (!data) {
			dev_err(dev,
				"failed to allocate memory for LED data\n");
			fwnode_handle_put(child);
			return -ENOMEM;
		}
		...
		ret = devm_led_classdev_register_ext(dev, &data->cdev,
						     &init_data);
		if (ret) {
				    ...
				    char *buf)
{
	...
	ret = r8c_exec_cmd(l->r8c, CMD_STS, NULL, cmdbuf, 4);
				     ...
				     const char *buf, size_t len)
{
	...
	ret = r8c_exec_cmd(l->r8c, CMD_STS, mode_name, NULL, 0);
				       ...
				       char *buf)
{
	...
	ret = r8c_exec_cmd(l->r8c, CMD_LED, NULL, cmdbuf, 4);
	if (ret || !strlen(cmdbuf))
		return 0;
				     ...
				     const char *buf, size_t len)
{
	...
	ret = r8c_exec_cmd(l->r8c, CMD_LED, buf, NULL, 0);
			     ...
			     unsigned long mode, void *cmd)
{
	...
	pr_info("r8c-leds: shutdown LEDs\n");
...
static int landisk_r8c_leds_probe(struct platform_device *pdev)
{
	...
	pr_info("R8C LEDs driver for I-O DATA LAN DISK series\n");
...
static struct platform_driver landisk_r8c_leds_driver = {
	...
	.driver = {
		.name = "leds-landisk-r8c",
		.of_match_table = landisk_r8c_leds_ids,
	},
};
module_platform_driver(landisk_r8c_leds_driver);

MODULE_DESCRIPTION("LEDS on R8C MCU driver for I-O DATA LAN DISK series");
