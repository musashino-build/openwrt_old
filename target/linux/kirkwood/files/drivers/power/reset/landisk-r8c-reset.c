			     ...
			     unsigned long mode, void *cmd)
{
	...
	ret = r8c_exec_cmd(r8c_rst->r8c, CMD_DUMMY, NULL, buf, 4);
	if (ret) {
	...
	ret = r8c_exec_cmd(r8c_rst->r8c, CMD_RESET, NULL, NULL, 0);
	if (ret)
		pr_err("r8c-reset: failed to execute reset %d\n", ret);
...
out:
	...
	if (!ret)
		ret = r8c_exec_cmd(r8c_rst->r8c, CMD_POFF, NULL, NULL, 0);
	if (ret)
		pr_err("r8c-poweroff: failed to execute poweroff %d\n", ret);
}
			     ...
			     char *buf)
{
	...
	ret = r8c_exec_cmd(r8c_rst->r8c, CMD_WOL, NULL, cmdbuf, 4);
	if (ret)
		return 0;
	...
	strncpy(buf, cmdbuf, 4);

	return ret;
}

static DEVICE_ATTR_RO(wol_flag);
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = r8c_exec_cmd(r8c_rst->r8c, CMD_PWR_AC, "3", cmdbuf, 4);
	if (ret)
		return 0;
				 ...
				 const char *buf, size_t len)
{
	...
	ret = r8c_exec_cmd(r8c_rst->r8c, CMD_PWR_AC, enable ? "3 1" : "3 0",
			   cmdbuf, 4);
	if (ret)
		return ret;

	/* R8C returns result of execution */
	if (strlen(cmdbuf) != 1 || cmdbuf[0] != '0')
		return -EINVAL;
...
static int landisk_r8c_reset_probe(struct platform_device *pdev)
{
	...
	pr_info("R8C reset driver for I-O DATA LAN DISK series\n");
...
static struct platform_driver landisk_r8c_reset_driver = {
	...
	.driver = {
		.name = "landisk-r8c-reset",
		.of_match_table = landisk_r8c_reset_ids,
	},
};
module_platform_driver(landisk_r8c_reset_driver);

MODULE_DESCRIPTION("R8C MCU reset driver for I-O DATA LAN DISK series");
