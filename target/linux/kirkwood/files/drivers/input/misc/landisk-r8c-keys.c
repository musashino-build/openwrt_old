...
static int landisk_r8c_keys_probe(struct platform_device *pdev)
{
	...
	pr_info("R8C keys driver for I-O DATA LAN DISK series\n");
	...
	return devm_r8c_register_event_notifier(dev, &k->nb);
}
...
static struct platform_driver landisk_r8c_keys_driver = {
	...
	.driver = {
		.name = "landisk-r8c-keys",
		.of_match_table = landisk_r8c_keys_ids,
	},
};
module_platform_driver(landisk_r8c_keys_driver);

MODULE_DESCRIPTION("Keys on R8C MCU driver for I-O DATA LAN DISK series");
